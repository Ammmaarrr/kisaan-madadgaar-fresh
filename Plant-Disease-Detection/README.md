...
## Run Notebooks on Google Colab from VS Code

You can execute local `.ipynb` notebooks on Google Colab’s GPU backend directly from VS Code.

- Install extensions:
    - Colab (publisher: Google)
    - Google Colab Keymap (optional)

- Via VS Code UI:
    - Open Extensions view → search "Colab" → install "Colab" by Google.

- Via PowerShell (if `code` CLI is available):
    - `code --install-extension google.colab`
    - `code --install-extension rebornix.colab-keymap`

- Connect your notebook:
    - Open your notebook in VS Code.
    - Use the kernel selector to choose Colab as compute.
    - Sign in to Google when prompted.
    - Cells run remotely on Colab; outputs render in VS Code.

- Notes:
    - `COLAB_TRAINING_NOTEBOOK.ipynb` includes a cell to upload `kaggle.json` and download datasets to `/content/datasets`.
    - Keep `kaggle.json` private; do not commit it. Regenerate from Kaggle settings if needed.
    - For long runs, save checkpoints to Drive or download artifacts regularly.
